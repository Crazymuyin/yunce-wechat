<template>
	<view class="load-more">
		...
		{{loadingType === 0 ? contentText.contentdown : loadingType === 1 ? contentText.contentrefresh : loadingType === 2 ? contentText.contentnomore : loadingType === 3 ? contentText.contentError : ''}}</text>
	</view>
</template>
...
<script>
	export default {
		...
		props: {
			loadingType: {
				//上拉的状态：0-loading前；1-loading中；2-没有更多了； 3-加载失败
				type: Number,
				default: 0
			},
			showImage: {
